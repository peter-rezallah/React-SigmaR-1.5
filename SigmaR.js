...
function createStream(v_window, v_params) {
    var C_Copyright = "2021. All rights reserved";
    ...
    var C_STATE_CONNECTION_TIMEOUT = 3;
    var C_STATE_CONNECTION_CONTINUED = 4;
	 ...
	 * Domain types
	 */
    var C_CONNECTION_LOCAL = 0; // host and dataserver
    var C_CONNECTION_SUB = 1; 
    var C_CONNECTION_PCLIENT = 2; 
    var C_CONNECTION_PSERVER = 3; 
    ...
    var C_MSG_ONTIMEOUT = 5;
    var C_MSG_ONCONTINUED = 6;
	...
	*/
    ...
    M.init = (v_window, v_params) => {
        ...
        M.cfgMaxMissedBeats = 5; //2
        M.cfgMaxRetries = 10; //3
        M.cfgStreamTimeout = 20; //10

        M.cfgFrameTimeout = 20; //20
        M.cfgConnectionTimeout = 20;
        M.cfgConnectionTimeoutMissedBeats = 0;
        ...
        if (M.connectionType == C_CONNECTION_PSERVER) {
            ...
            M.params.onTimeout = PS.onTimeout;
            M.params.onContinued = PS.onContinued;
    ...
    M.detectConnectionType = function () {
        ...
        M.log(LL_TRACE2, "Detected domain type: " + M.domainTypeToString(M.connectionType));
    };
    ...
    M.disconnect = function () {
        ...
        if (M.connectionTimeoutTimer) {
            clearInterval(M.connectionTimeoutTimer);
            M.connectionTimeoutTimer = 0;
        }
    ...
    M.timeout = function () {
        M.state = C_STATE_CONNECTION_TIMEOUT;
        if (M.params.onTimeout)
            M.params.onTimeout();       
    }

    M.continued = function () {
        M.state = C_STATE_CONNECTION_CONTINUED;
        if (M.params.onContinued)
            M.params.onContinued();       
    }
    ...
    M.onHeartTimer = function () {
        ...
        if (M.streamMode == C_MODE_STREAMING) {
            if (++M.missedBeats == M.cfgMaxMissedBeats) {
                ...
                M.dropConnection();
                //alert("*************DropConnection**********" + M.missedBeats + "M.cfgMaxMissedBeats=" + M.cfgMaxMissedBeats)
            }
			/*else
			{
    ...
    M.sendNextRequestXmlHttp = function (v_stream) {
        ...
        // Build the URL
        var v_url = M.server + "/signalr/connect?transport=serverSentEvents&connectionId=" + M.channelId + "&stream=1&connectionData=[{\"name\":\"Stream\"}]&client=FLEX";
        //v_url += "&get=" + M.nextRecvSeq;
        if (v_stream && M.conncted) {
            //v_url += "&stream=1";
            v_url = M.server + "/signalR/?transport=serverSentEvents&connectionId=" + M.channelId + "&messageId=" + M.MessageId + "&stream=1&connectionData=[{'name':\'Stream\'}]&client=FLEX";

        }
        else if (!v_stream && M.conncted) {
            v_url = M.server + "/signalR/?transport=serverSentEvents&connectionId=" + M.channelId + "&messageId=" + M.MessageId + "&connectionData=[{'name':\'Stream\'}]&client=FLEX";
        }
        else {
            var v_url = M.server + "/signalr/connect?transport=serverSentEvents&connectionId=" + M.channelId + "&stream=1&connectionData=[{\"name\":\"Stream\"}]&client=FLEX";
        }
    ...
    M.setConnectionTimeoutTimer = function () {
        M.connectionTimeoutTimer = setInterval(M.connectionTimeoutHeartBeat, 1000);
    }
    ...
    M.sendNextRequestIFrame = function () {
        ...
        v_url = M.server + "/signalr/connect?transport=serverSentEvents&connectionId=" + M.channelId + "&stream=1&connectionData=[{\"name\":\"Stream\"}]&client=FLEX";
        if (M.conncted) v_url = M.server + "/signalR/?transport=serverSentEvents&connectionId=" + M.channelId + "&messageId=" + M.MessageId + "&stream=1&connectionData=[{\"name\":\"Stream\"}]&client=FLEX";
    ...
    M.onOpenReply = function () {
        ...
        var v_data = M.stmRequest.responseText;
        M.evalReply(v_data);

        // Send the next streaming request, and also flush any
        // new commands that have been added in the meantime.

        if (M.state == C_STATE_CONNECTED || M.state == C_STATE_CONNECTION_CONTINUED) {
            M.sendNextRequest();
            //M.sendFlush();
        }
    };
    ...
    M.onStmReply = function () {
        var v_status, v_text, v_pos, v_data, res_text;
        ...
        if (M.stmRequest.readyState == 3) {
	        /*v_pos = M.stmRequest.responseText.lastIndexOf(
				"\n\n", M.stmRequest.responseText.length);*/

            res_text = M.stmRequest.responseText.replace(/\n\s+\n|\n/g, "");
            v_pos = res_text.lastIndexOf(";", res_text.length);


            //alert("v_pos="+v_pos+" ,M.stmPosition="+M.stmPosition+",text="+res_text)

            if (v_pos > M.stmPosition) {
                v_data = res_text.substr(M.stmPosition, v_pos - M.stmPosition);
                //alert("v_data="+v_data);
                M.log(LL_TRACE,"recieved=" + v_data);
                M.stmPosition = v_pos;
                M.evalReply(v_data);
        ...
        if (M.state == C_STATE_CONNECTED || M.state == C_STATE_CONNECTION_CONTINUED || M.conncted == true) {
            M.sendNextRequest();
        }
    };
    ...
    M.sendFlush = function () {
        ...
        // Ok, send it
        if (M.conncted) {
            v_url = M.server + "/signalR/send?transport=serverSentEvents&connectionId=" + M.channelId + "&connectionData=[{\"name\":\"Stream\"}]&client=FLEX";
    ...
    M.connectionTimeoutHeartBeat = function () {
        /*if (M.MessageId == null || M.MessageId == "")
            return;*/

        if (M.MessageId == M.LastMessageId) {
            M.cfgConnectionTimeoutMissedBeats = M.cfgConnectionTimeoutMissedBeats + 1;
            if (M.cfgConnectionTimeoutMissedBeats >= M.cfgConnectionTimeout && M.state != C_STATE_CONNECTION_TIMEOUT ) {
            ...
            M.LastMessageId = M.MessageId;
            M.cfgConnectionTimeoutMissedBeats = 0;
            if (M.state == C_STATE_CONNECTION_TIMEOUT) {
                M.continued();
            }            
        }
    }
    ...
    M.evalReply = function (v_text) {
        var sArray = v_text.split(";")
        sArray.forEach((s) => { if (s != null && s !== "") eval("M.context." + s); });
    ...
    M.cmdHeartbeat = function (v_time) {
        ...
        if (M.streamMode == C_MODE_UNKNOWN) {
            ...
            //alert("Detected streaming connection");
            M.streamMode = C_MODE_STREAMING;


        }
        //alert("*********Streaming********"+M.streamMode)
    ...
    PC.timeout = function () {
        if (M.xdframe)
            M.sendXDMessage({ type: C_MSG_ONTIMEOUT });
    }

    PC.continued = function () {
        if (M.xdframe)
            M.sendXDMessage({ type: C_MSG_ONCONTINUED });
    }

    PC.onServerTimeout = function () {
        M.state = C_STATE_CONNECTION_TIMEOUT;
        if (M.params.onTimeout)
            M.params.onTimeout();
    }

    PC.onServerContinued = function () {
        M.state = C_STATE_CONNECTION_CONTINUED;
        if (M.params.onContinued)
            M.params.onContinued();
    }
    ...
    PC.sendFlush = function (v_bForceSend) {
        // Must be connected
        if (M.state != C_STATE_CONNECTED && M.state != C_STATE_CONNECTION_CONTINUED && !v_bForceSend) {
        ...
        for (var v_i = 0; v_i < M.sendQueue.length; v_i++) {
            ...
            M.sendXDMessage({
                type: C_MSG_SEND, msgtype: v_msg.m_type,
    ...
    PC.onMessage = function (v_event) {
        ...
        switch (v_data.type) {
            ...
            case C_MSG_ONDISCONNECTED:
                PC.onServerDisconnected();
                break;
            case C_MSG_ONTIMEOUT:
                PC.onServerTimeout();
                break;
            case C_MSG_ONCONTINUED:
                PC.onServerContinued();
                break;
            case C_MSG_ONMSG:
                if (M.params.onMsg)
    ...
    PS.onTimeout = function () {
        M.sendXDMessage({ type: C_MSG_ONTIMEOUT });
    };

    PS.onContinued = function () {
        M.sendXDMessage({ type: C_MSG_ONCONTINUED });
    };
    ...
    PS.onMsg = function (v_type, v_header, v_body) {
        ...
        M.sendXDMessage({
            type: C_MSG_ONMSG, msgtype: v_type,
    ...
    switch (M.connectionType) {
        case C_CONNECTION_PCLIENT:
            ...
            v_pub.timeout = PC.timeout;
            v_pub.continued = PC.continued;
            break;
        ...
        case C_CONNECTION_PSERVER:
            ...
            v_pub.timeout = M.timeout;
            v_pub.continued = M.continued;
...
(function () {
	...
	*/
    ...
    M.init = function (v_callback, v_window) {
        ...
        v_cb.onTimeout = M.onStreamTimeout;
        v_cb.onContinued = M.onStreamContinued;
    ...
    M.softDisconnect = function () {
        ...
        M.onStreamDisconnected();
    }

    M.timeout = function () {
        M.setConnectionStatus(C_DatState.Timeout, C_DatError.None);
        v_pub.stream.timeout();
    };

    M.continued = function () {
        M.setConnectionStatus(C_DatState.Continued, C_DatError.None);
        v_pub.stream.continued();
    };
	...
	*/
    ...
    M.onStreamTimeout = function () {

        M.log(LL_INFORMATION, "TimeOut");
        M.m_timeout = true;
        M.setConnectionStatus(C_DatState.Timeout, C_DatError.None);
        if (v_pub.i.callback.onTimeout)
            v_pub.i.callback.onTimeout();

    }

    M.onStreamContinued = function () {
        M.log(LL_INFORMATION, "Continued");
        M.m_timeout = false;
        M.setConnectionStatus(C_DatState.Continued, C_DatError.None);
        if (v_pub.i.callback.onContinued)
            v_pub.i.callback.onContinued();
    }
    ...
    M.startReconnectTimer = function () {
        ...
        // Start the timer
        M.timer = M.win.setTimeout(M.onReconnectTimer, 5 * 1000);
    };
	...
	*/
    ...
    v_pub.continued = M.continued;
    v_pub.disconnect = M.disconnect;
    v_pub.softDisconnect = M.softDisconnect;
...
(function () {
	...
	*/
    ...
    M.goDom = function (v_request, v_elem, v_onUpdate, v_onStatus, v_source, v_symbol, v_field, v_dataFormat, v_colIndex, v_rowIndex, v_conflation) {
        var v_n = 0; var v_m = 0;
        var F_getAttr = function (v_name, v_transform, v_default) {
            try {
        ...
        try {
            v_source = F_getAttr("data-source", C_undefined, v_source);
            v_symbol = F_getAttr("data-symbol", C_undefined, v_symbol);
            if (!v_symbol) v_symbol = F_getAttr("data-item", C_undefined, v_symbol);
            v_field = F_getAttr("data-field", C_undefined, v_field);
            v_colIndex = F_getAttr("data-colindex", C_undefined, v_colIndex);
            v_rowIndex = F_getAttr("data-rowindex", C_undefined, v_rowIndex);
            v_dataFormat = F_getAttr("data-dataformat", C_undefined, v_dataFormat);
            v_conflation = F_getAttr("data-conflation", C_undefined, v_conflation);
            v_onUpdate = F_getAttr("data-onupdate", "new Function('elem','dataItem','fieldName','value','bImage',a)", v_onUpdate);
            v_onStatus = F_getAttr("data-onstatus", "new Function('elem','dataItem','state','error','errorMessage',a)", v_onStatus);
            ...
            if ((v_n > 0 || v_m > 0) && v_source && v_symbol && v_field) {
                if (v_dataFormat == data.DatFormat.Table) {
                    ...
                    table.getCell(row, col).push({
                        m_elem: v_elem, m_type: data.DatFormat.Table,
                        F_onUpdate: v_onUpdate, F_onStatus: v_onStatus
                    });
                }
                else {
                    // record
                    var rec = M.getRecord(v_request, v_source, v_symbol, v_conflation);
                    rec.getField(v_field).push({
                        m_elem: v_elem, m_type: data.DatFormat.Record,
                        F_onUpdate: v_onUpdate, F_onStatus: v_onStatus
                    });
                }
            ...
            if (bRecurse)
                for (var i = 0; i < v_elem.childNodes.length; i++) {
                    if ((v_elem.childNodes[i]) && (v_elem.childNodes[i].nodeType == 1))
                        M.goDom(v_request, v_elem.childNodes[i],
                            v_onUpdate, v_onStatus, v_source, v_symbol,
                            v_field, v_dataFormat, v_colIndex, v_rowIndex, v_conflation);
                }
        }
        finally {
            //_this.lv--;
    ...
    M.parseData = function (v_Data, v_options) {

        var v_request = { m_records: {}, m_tables: {} };
        if (!v_options) v_options = {};
        if (!v_options.elem) {
            M.log(LL_INFORMATION, "Using default document Body ");
            v_options.elem = document.getElementsByTagName("BODY")[0];
        }
        if (!v_options.onUpdate) {
            M.log(LL_INFORMATION, "Using default Data update handler");
            v_options.onUpdate = M.defaultUpdateHandler;
        }
        if (!v_options.onStatus) {
            M.log(LL_INFORMATION, "Using default Data state handler");
            v_options.onStatus = M.defaultStateHandler;
        }


        M.goData(v_request, v_Data, v_options.elem, v_options.onUpdate, v_options.onStatus,
            v_options.source, v_options.symbol, v_options.field,
            v_options.dataFormat, v_options.colCount, v_options.rowCount,
            v_options.colIndex, v_options.rowIndex,
            v_options.conflation);


        // now, request all the items....
        M.log(LL_INFORMATION, "Data parsed ok. Requesting data...");
    ...
    var C_undefined;
    M.goData = function (v_request, v_data, v_elem, v_onUpdate, v_onStatus, v_source, v_symbol, v_field,
        v_dataFormat, v_colCount, v_rowCount, v_colIndex, v_rowIndex, v_conflation) {

        var _v_data = v_data;
        for (var i = 0; i < _v_data.length; i++) {
            v_source = _v_data[i]["source"];
            v_symbol = _v_data[i]["symbol"];
            if (!v_symbol) 
                v_symbol = _v_data[i]["item"];
            v_field = _v_data[i]["field"];
            v_colCount = _v_data[i]["colCount"];
            v_rowCount = _v_data[i]["rowCount"];
            v_colIndex = _v_data[i]["colIndex"];
            v_rowIndex = _v_data[i]["rowIndex"];
            v_dataFormat = _v_data[i]["dataFormat"];
            v_conflation = _v_data[i]["conflation"];
            /*v_onUpdate = (_v_data[i]["onupdate"] !== "") ?
                "new Function('elem','dataItem','fieldName','value','bImage',a)" :
                _v_data[i]["onupdate"] ;*/
            v_onStatus = _v_data[i]["onstatus"];
            v_elem = (!_v_data[i]["elem"]) ? v_elem : _v_data[i]["elem"];

            if (!v_field && v_colIndex && v_rowIndex && v_dataFormat == data.DatFormat.Table)
                v_field = "R" + v_rowIndex + "C" + v_colIndex;


            if (v_source && v_symbol && v_field  ) {
                if (v_dataFormat == data.DatFormat.Table && v_colCount && v_rowCount ) {
                    // table
                    /*var rAt = 0;
                    var cAt = v_field.indexOf("C", rAt + 1);
                    var row = parseInt(v_field.substr(rAt + 1, cAt - rAt - 1));
                    ...
                    for (var row = 0; row < v_rowCount; row++) {
                        for (var col = 0; col < v_colCount; col++) {
                            table.getCell(row, col).push({
                                m_elem: v_elem, m_type: data.DatFormat.Table,
                                F_onUpdate: v_onUpdate, F_onStatus: v_onStatus
                            });
	...
	*/
    M.onCellChanged = function (v_table, v_row, v_col, v_value, v_bIsImage) {
        ...
        for (var v_k in v_cell) {
            ...
            v_cell[v_k].F_onUpdate.call(v_cell[v_k].m_elem, v_cell[v_k].m_elem, v_table, v_row , v_col, v_value);
        }
    };
    ...
    v_pub.parseData = M.parseData;
    v_pub.cancelData = M.cancelData;
